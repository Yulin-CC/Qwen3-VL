...
1. 维护者双击 `build/一键打包.bat`，产物在 `build/0-dist/GroundingReview-portable-*.zip`（当前示例：`v0.4.0`）
...
- 每个目标右侧一般展示 **最多 5 条**候选描述（含已选虚线）+ 末尾「保留源标签」；**亮绿**为当前 caption 已选；其它 caption 已用且未选的精确重复先**隐藏**；已选冲突以**虚线**标出便于点灭；同组近义不并列；切 caption 不重排已选
- **虚线**短语表示已被其它 caption 占用，尽量换一条，避免重复
- caption **不可为空**：未选目标描述时切换 W/S 或点 tab 会提示补选
- 不同目标禁止写成「A is also B」这类合并歧义句；同目标才可用同位语
- 刷新描述会尽量保留当前 caption 已选短语，只重刷其余空位（单次模型、先英文后补中文）；也可左键选中右侧目标卡后**右键**刷新
