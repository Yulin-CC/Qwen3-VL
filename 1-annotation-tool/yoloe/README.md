...
1. 维护者双击 `build/一键打包.bat`，产物在 `build/0-dist/GroundingReview-portable-*.zip`
2. 同事解压后双击 `start.bat` → 浏览器打开 `http://localhost:8090`
...
├── draft/           # 运行时生成（objects / descriptions / captions）
...
| `rules/caption_rules.md` | caption 句式组合规则（短语锁定、同 obj 不拆人） |
