...
**便携包：** 本轮（v0.6.11）不搬；审阅仍用 `1-start_review.bat`。yoloe 最后一版为 `GroundingReview-portable-v0.5.6.zip`。
...
3. 右侧「标签概览」按类别成卡；点图中任意目标，右侧弹出漫画对话框显示该目标全部描述。对话框「删除」或 **Delete** 删该目标（与「目标描述」相同）。**G** 改描述：点对话框中的描述后 **Delete** 删该条；未选描述时 **Delete** 仍删目标；点右侧描述后 **B** 替换 / **F** 新增。点右侧一条描述后 **V** 点亮/点灭图中目标，**F** 确认并退出（点灭后空槽用源标签，不强制补其它描述，切图不拦）。每张类别卡上的「整理」只排该类芯片
4. 审阅后导出 `jsons-GD`（当前文件或整个文件夹）

快捷键：A/D 切图 · W/S 切 caption · **R** 框显隐 · **T** 匹配上一张 · **F** 确认（匹配 / 改描述新增 / 点亮写入）· 标签概览 **G** 改描述 · **V** 点亮/点灭 · **Delete** 删选中目标（G 下若已点对话框描述则只删该条）· **B** 替换。其它见 [`z-others/标注规则.md`](./z-others/标注规则.md)。
