...
state = {
    ...
    "workers": int(os.environ.get("DESCRIBE_WORKERS", "4")),
    "timeout": int(os.environ.get("VLLM_TIMEOUT", "600")),
}
...
def _apply_service_from_request(body=None):
    """用请求里的 service_id 覆盖当前服务；没有则保持界面已选。"""
    body = body if body is not None else (request.json or {})
    sid = str((body or {}).get("service_id") or "").strip()
    if not sid:
        sid = str(state.get("service_id") or "").strip()
    if not sid:
        raise ValueError("请先在顶栏选择模型服务")
    svc = get_service(sid)
    if not svc:
        raise ValueError(f"未知模型服务: {sid}")
    _apply_service(svc)
    return svc
...
def cluster_label_chips(classes, phrases):
    """用 vLLM 把描述芯片按语义分组；classes 可空（按单张类别卡整理）。"""
    from vllm_client import extract_json_object
    ...
    for c in classes or []:
        if isinstance(c, dict):
            key = _chip_key(c.get("key") or c.get("label"))
        ...
        if not key or key in seen_c:
            continue
        seen_c.add(key)
        class_items.append({"key": key, "label": label or key, "zh": zh})

    phrase_items = []
    seen_p = set()
    for p in phrases or []:
        if isinstance(p, dict):
            key = _chip_key(p.get("key") or p.get("phrase"))
            phrase = str(p.get("phrase") or key).strip()
            zh = str(p.get("zh") or "").strip()
        else:
            key = _chip_key(p)
            phrase = str(p).strip()
            zh = ""
        if not key or key in seen_p:
            continue
        seen_p.add(key)
        phrase_items.append({"key": key, "phrase": phrase or key, "zh": zh})

    if not class_items and not phrase_items:
        return {"class_groups": [], "phrase_groups": []}

    def fmt_list(items, name_key, extra_key):
        lines = []
        for i, it in enumerate(items, 1):
            name = it.get(name_key) or it["key"]
            extra = it.get(extra_key) or ""
            if extra and extra.lower() != str(name).lower():
                lines.append(f"{i}. {name} ({extra})")
            else:
                lines.append(f"{i}. {name}")
        return "\n".join(lines) if lines else "(无)"

    n_ph = len(phrase_items)
    prompt = (
        "把芯片按语义分组，同类挨在一起。例：上衣/夹克/衬衫；帽/盔/口罩；裤/牛仔裤。"
        "不要按颜色分组。只返回 JSON。\n"
        f"{fmt_list(phrase_items, 'phrase', 'zh')}\n"
        '{"groups":[{"title":"上装","ids":[1,2]}]}'
    )
    # 短输出：约 20 tokens/组，避免模型拖长推理
    max_tokens = min(512, 96 + n_ph * 16)
    print(
        f"[cluster_labels] n={n_ph} {state.get('service_id') or '-'} "
        f"{state.get('base_url')} model={state.get('model')} max_tokens={max_tokens}",
        flush=True,
    )
    raw = _client().text(
        prompt,
        system="JSON only. No markdown, no thinking.",
        temperature=0,
    ...
    def map_groups(raw_groups, items):
        used = set()
        ...
        for g in raw_groups or []:
            if not isinstance(g, dict):
                continue
            title = str(g.get("title") or "").strip() or "其它"
            ids = g.get("ids") if g.get("ids") is not None else g.get("items")
            keys = []
            for x in ids or []:
                k = None
                ...
                if k and k not in used:
                    ...
                    used.add(k)
            if keys:
                out.append({"title": title, "keys": keys})
        leftover = [it["key"] for it in items if it["key"] not in used]
        if leftover:
            out.append({"title": "其它", "keys": leftover})
        return out

    phrase_raw = data.get("phrase_groups")
    if phrase_raw is None:
        phrase_raw = data.get("groups")
    return {
        "class_groups": map_groups(data.get("class_groups"), class_items) if class_items else [],
        "phrase_groups": map_groups(phrase_raw, phrase_items) if phrase_items else [],
    }


def _merge_label_clusters(old, new, scope_keys):
    """只替换本次整理范围内的芯片顺序，其它类别保持。"""
    scope = {_chip_key(k) for k in (scope_keys or []) if k}
    ...
    def keep(groups):
        ...
        for g in groups or []:
            if not isinstance(g, dict):
                continue
            keys = [k for k in (g.get("keys") or []) if _chip_key(k) not in scope]
            if keys:
                out.append({"title": str(g.get("title") or "其它"), "keys": keys})
        return out

    phrase_groups = keep(old.get("phrase_groups")) + list(new.get("phrase_groups") or [])
    class_groups = old.get("class_groups") or []
    if new.get("class_groups"):
        class_groups = new["class_groups"]
    return {"class_groups": class_groups, "phrase_groups": phrase_groups}
...
def generate_start():
    ...
    if not os.path.isdir(root):
        return jsonify({"error": f"目录不存在: {root}"}), 400
    try:
        write_draft_config(root, {"rules_scene": get_rules_scene()})
    except Exception:
        pass
...
def _load_item_payload(stem):
    ...
    payload = {"meta": meta, "captions": caps, "descriptions": descs}
    clusters = load_json(
        os.path.join(draft_dir(_root()), "label_clusters", f"{stem}.json")
    )
    if clusters:
        payload["label_clusters"] = clusters
    return payload
...
def serve_image(stem):
    ...
    resp = send_from_directory(img_dir, meta["image"])
...
@app.route("/api/cluster_labels", methods=["POST"])
@app.route("/api/cluster_labels/<stem>", methods=["POST"])
def cluster_labels_api(stem=None):
    """一键整理标签概览芯片顺序（不改 phrases / captions）。可只整理一个类别。"""
    if not _root():
        return jsonify({"error": "未配置数据集"}), 400
    body = request.json or {}
    stem = (stem or body.get("stem") or "").strip()
    if not stem:
        return jsonify({"error": "未指定文件"}), 400
    try:
        svc = _apply_service_from_request(body)
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    phrases = body.get("phrases") or []
    classes = body.get("classes") or []
    try:
        new = cluster_label_chips(classes, phrases)
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": f"整理失败: {e}"}), 500
    path = os.path.join(draft_dir(_root()), "label_clusters", f"{stem}.json")
    old = load_json(path) or {}
    scope = []
    for p in phrases:
        if isinstance(p, dict):
            scope.append(p.get("key") or p.get("phrase") or "")
        else:
            scope.append(p)
    clusters = _merge_label_clusters(old, new, scope)
