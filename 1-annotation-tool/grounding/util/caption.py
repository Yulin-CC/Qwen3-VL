...
# @Author: AI产品研发组
# @Date: 2026-08-20
# @Description: 邻域组 caption，整组覆盖全部目标描述（不够则追加）
# @Command: python util/caption.py --dataset /path/to/data
"""
...
from rules_io import get_scene_policy, load_caption_rules_text, set_rules_scene
...
def pick_groups(objects, width, height, n_captions=5, grid=3, seed=None):
    ...
    for gi in range(n_local):
        ...
        k = rng.randint(3, min(COVER_CHUNK, n)) if n >= 3 else n
        groups.append({
    ...
    for g in groups:
        if any(set_overlap(g["obj_ids"], u["obj_ids"]) > 0.7 for u in unique):
            if g["strategy"] == "global_random":
                ...
                anchor_idx = rng.randrange(n)
                g = {
                    "obj_ids": nearest_group(objects, anchor_idx, len(g["obj_ids"])),
                    "strategy": "anchor_neighbor",
    ...
    while len(unique) < n_captions and n > 0 and attempts < 40:
        attempts += 1
        anchor_idx = rng.randrange(n)
        k = rng.randint(1, min(COVER_CHUNK, n))
        g = {
            "obj_ids": nearest_group(objects, anchor_idx, k),
            "strategy": "anchor_neighbor",
            "grid_cell": -1,
    ...
    while len(unique) < max(n_captions, 5) and unique:
        g = unique[len(unique) % len(unique)]
        unique.append({
...
def pick_phrases_for_group(objects, desc_map, obj_ids, rng, avoid_by_obj=None):
    """从目标描述 phrases 中取一条；优先尚未在其它 caption 出现过的短语。"""
    ...
    for oid in obj_ids:
        ...
        base = strong or opts or chosen or [obj["label"]]
        avoid = avoid_by_obj.get(int(oid)) or set()
        avoid_l = {str(a).strip().lower() for a in avoid}
        fresh = [x for x in base if x.strip().lower() not in avoid_l] if avoid_l else []
        if avoid_l and not fresh:
            continue
        pool = fresh or base
...
def build_caption_with_llm(client: VLLMClient, phrases, rules=None, vary_structure=False):
    ...
    raw = client.text(
        ...
        max_tokens=min(1024, max(256, 48 * max(1, len(phrases)) + 64)),
    )
...
def build_one_caption_entry(objects, desc_map, obj_ids, strategy, grid_cell,
    ...
    if fixed_phrases is not None:
        ...
        if not phrases:
            return None
        obj_ids = [p["obj_id"] for p in phrases]
...
def unused_phrase_obj_ids(objects, desc_map, captions):
    """仍有未入 caption 的外观短语的目标（弱占位短语不算）。"""
    used = used_phrases_by_obj(captions)
    out = []
    for obj in objects:
        try:
            oid = int(obj["obj_id"])
        ...
        opts = [
            x.strip() for x in ((desc_map.get(oid) or {}).get("phrases") or [])
            if isinstance(x, str) and x.strip()
        ]
        strong = [x for x in opts if not _is_weak_phrase(x, label)]
        pool = strong or opts
        if not pool:
            continue
        have = {p.lower() for p in (used.get(oid) or set())}
        if any(p.lower() not in have for p in pool):
            out.append(oid)
    return out


def _chunk_cover_ids(objects, priority_ids):
    """从优先集合里取一簇邻近目标，最多 COVER_CHUNK 个。"""
    if not priority_ids:
        return []
    id2idx = {o["obj_id"]: i for i, o in enumerate(objects)}
    anchor = next((oid for oid in priority_ids if oid in id2idx), None)
    if anchor is None:
        return list(priority_ids)[:COVER_CHUNK]
    near = nearest_group(objects, id2idx[anchor], min(COVER_CHUNK, len(objects)))
    want = set(priority_ids)
    chosen = [oid for oid in near if oid in want]
    for oid in priority_ids:
        if oid not in chosen:
            chosen.append(oid)
        if len(chosen) >= COVER_CHUNK:
            break
    return chosen[:COVER_CHUNK]


def extend_captions_to_cover(objects, desc_map, captions, client, rng):
    """初始条数不够覆盖时追加 caption：先补未入镜目标，再补未用短语。"""
    if not objects:
        return captions
    max_extra = max(8, len(objects) * 3)
    for _ in range(max_extra):
        miss_obj = [o["obj_id"] for o in objects if o["obj_id"] not in covered_obj_ids(captions)]
        if miss_obj:
            chosen, strategy = _chunk_cover_ids(objects, miss_obj), "cover_objects"
        else:
            miss_ph = unused_phrase_obj_ids(objects, desc_map, captions)
            if not miss_ph:
                break
            chosen, strategy = _chunk_cover_ids(objects, miss_ph), "cover_phrases"
        entry = build_one_caption_entry(
            objects, desc_map, chosen, strategy, -1,
            sentence_id=len(captions), client=client, rng=rng,
            avoid_by_obj=used_phrases_by_obj(captions),
        )
        if not entry:
            break
...
def caption_one_image(dataset_root, stem, client=None, n_captions=5, seed=None, force=False):
    ...
    if get_scene_policy().get("caption_cover_all"):
        captions = extend_captions_to_cover(objects, desc_map, captions, client, rng)
...
def captions_need_rebuild(dataset_root, stem) -> bool:
    ...
    desc_map = load_desc_map(dataset_root, stem)
    obj_path = os.path.join(draft_dir(dataset_root), "objects", f"{stem}.json")
    meta = load_json(obj_path) or {}
    objects = meta.get("objects") or []
    captions = caps.get("captions") or []
    if objects and get_scene_policy().get("caption_cover_all") and (
        any(o["obj_id"] not in covered_obj_ids(captions) for o in objects)
        or unused_phrase_obj_ids(objects, desc_map, captions)
    ):
        return True
...
def caption_dataset(dataset_root, base_url, model, n_captions=5, limit=None,
                    force=False, no_llm=False, timeout=600, progress_cb=None):
    """
    ...
    client = None if no_llm else VLLMClient(
        base_url=base_url, model=model, timeout=timeout, enable_thinking=False
    )
